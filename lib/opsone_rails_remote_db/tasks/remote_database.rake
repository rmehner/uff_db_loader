...
namespace :remote_database do
  ...
  task load: :environment do
    prompt = TTY::Prompt.new
    environment = prompt.select("Which environment should we get the dump from?", OpsoneRailsRemoteDb.config.environments)
    result_file_path = OpsoneRailsRemoteDb.dump_from(environment)
    ...
    command_successful = system(OpsoneRailsRemoteDb.restore_command(database_name, result_file_path))
    raise "Command did not run succesful: #{OpsoneRailsRemoteDb.restore_command(database_name, result_file_path)}" unless command_successful
